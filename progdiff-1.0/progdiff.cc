...
void fix(string &buf) {
  ...
  if (kde < buf.size()) res+="...";
  if (!eol) res += " (missing end-of-line character)";
  buf = res;
}
...
int main(int argc, char **argv){
  if (argc<3) { printf("Usage: %s file1 file2 [label1 [label2]]\n",argv[0]); return 1; }
  ifstream f1(argv[1]);
  ifstream f2(argv[2]);
  if (!f1) { printf("Cannot open file %s.\n",argv[1]); return 1; }
  if (!f2) { printf("Cannot open file %s.\n",argv[2]); return 1; }

  memset(l1,0,sizeof(l1)); memset(l2,0,sizeof(l2));
  strncpy(l1,(argc>=4)?argv[3]:argv[1],8);
  strncpy(l2,(argc>=5)?argv[4]:argv[2],8);
  if (!strcmp(l1,l2)) { strcpy(l1,"first"); strcpy(l2,"second"); }
...
//      printf("         %04d: %s\n",row,buf1);
  ...
  if (errors>10) printf("(skipping the next %d errors)\n",errors-10);
  
  if ((!f1) && (f2)) printf("%s terminated, %s continues:\n",l1,l2);
  if ((f1) && (!f2)) printf("%s terminated, %s continues:\n",l2,l1);
  int nrows=0;
  ...
  if (nrows > 10) printf("(skipping next %d lines)\n",nrows-10);
  return 0;
}
